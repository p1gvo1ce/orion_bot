...
async def ensure_game_roles_channel(guild):
    game_roles_channel_ids = await read_from_guild_settings_db(guild.id, "game_roles_channel_id")
    ...
    if not game_roles_channel_ids:
        ...
        await write_to_guild_settings_db(guild.id, "game_roles_channel_id", f"id{game_roles_channel.id}")
    else:
...
async def add_game_in_game_roles_channel(roles, guild):
    ...
    if isinstance(roles, set):
        for role in roles:
            message = await game_roles_channel.send(f"{role.mention}")
            await message.add_reaction("🎮")
    else:
        message = await game_roles_channel.send(f"{roles.mention}")
        await message.add_reaction("🎮")